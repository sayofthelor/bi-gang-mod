...
{
	var options:Array<String> = ['Notes', 'Controls', 'Preferences', 'Speaker Char Select'];
	...
	override function update(elapsed:Float) {
		...
		if (controls.ACCEPT) {
			...
			switch(options[curSelected]) {
				...
				case 'Speaker Char Select':
					openSubState(new SpeakerCSSubstate());
			}
		}
	}
	...
	override function update(elapsed:Float)
	{
		...
		if(!usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					case 'GF':
						FlxG.save.data.scs = 0;
						MusicBeatState.switchState(new MainMenuState());
					case 'Ruby':
						FlxG.save.data.scs = 1;
						MusicBeatState.switchState(new MainMenuState());
					case 'Nameless Remix Mod GF':
						FlxG.save.data.scs = 2;
						MusicBeatState.switchState(new MainMenuState());
					case 'Yuri':
						FlxG.save.data.scs = 3;
						MusicBeatState.switchState(new MainMenuState());
					case 'Poppy':
						FlxG.save.data.scs = 4;
						MusicBeatState.switchState(new MainMenuState());
				}
				FlxG.sound.play(Paths.sound('scrollMenu'));
				reloadValues();
...
class SpeakerCSSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var options:Array<String> = [
		'GF',
		'Ruby',
		'Nameless Remix Mod GF',
		'Yuri',
		'Poppy'
	];
	static var noCheckbox:Array<String> = [];
